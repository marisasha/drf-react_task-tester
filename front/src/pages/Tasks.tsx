...
export default function Page() {
  ...
  const taskToArchive = useSelector((state: any) => state.taskToArchive);
  const userId = utils.LocalStorage.get("user_id"); // Получаем user_id из локального хранилища
  ...
  async function handleToArchive(taskId: number) {
    await components.constructorWebAction(
      dispatch,
      constants.taskDelete,
      `${constants.host}/api/task/to/archive/${taskId}`,
      "POST",
    ).then
      setData((prevData) => prevData.filter((item) => item.id !== taskId));
  }
              ...
              <div key={item.id} className="rounded text-slate-800 w-full">
                <div className="category flex flex-col bg-white rounded w-full h-80 mb-10 pt-10">
                  ...
                  <div className="flex gap-x-2 justify-center">
                    ...
                    <div className="flex justify-center mt-5">
                        <button
                          onClick={() => handleToArchive(item.id)}
